...
TAG=${EXPERIMENT}-${TRIAL}-ensemble  # required
...
argo submit --from workflowtemplate/offline-diags \
    -p ml-model="gs://vcm-ml-experiments/n2f-pire-stable-ml/2022-07-06/decrease-rad-lr-rad-flux-ensemble/trained_models/radiative_fluxes/" \
    -p training_config="$( yq . $TMP_CONFIG )" \
    -p training_data_config="$( yq . train-data.yaml )" \
    -p test_data_config="$( yq . test-data.yaml )" \
    -p offline-diags-output="gs://vcm-ml-experiments/n2f-pire-stable-ml/2022-07-06/decrease-rad-lr-rad-flux-ensemble/offline_diags/radiative_fluxes/" \
